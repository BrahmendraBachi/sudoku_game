...
class Third_Level:
    ...
    def findThirdLevel(self):

        for i in range(9):
            if len(self.r_void_cells[i]) == 3:
                 if self.fillValueForThirdLevel(self.r_void_cells[i], 'r'):
                     return True
            if len(self.c_void_cells[i]) == 3:
                if self.fillValueForThirdLevel(self.c_void_cells[i], 'c'):
                    return True
        for k_str in self.k_strs:
            if len(self.m_grid_void_cells[k_str]) == 3:
                if self.fillValueForThirdLevel(self.m_grid_void_cells[k_str], 'm_g'):
                    return True
        return False

    def fillValueForThirdLevel(self, cells, rowORcolORgrid):
        grid = self.grid
        nums = []
        if rowORcolORgrid == 'r':
            r = cells[0][0]
    ...
    def canFill(self, cells, nums):
        ...
        grid = self.grid
        isValid = False
        cell_A, cell_B, cell_C = cells[0], cells[1], cells[2]

        for num in nums:

            if self.checkNumberForOneCell(cell_A, cell_B, cell_C, num):
                r, c = cell_A[0], cell_A[1]
                isValid = True
                break

            if self.checkNumberForOneCell(cell_B, cell_A, cell_C, num):
                r, c = cell_B[0], cell_B[1]
                isValid = True
                break

            if self.checkNumberForOneCell(cell_C, cell_A, cell_B, num):
    ...
    def checkNumberForOneCell(self, cell_A, cell_B, cell_C, num):
        
        return self.validate.checkNumber(cell_A[0], cell_A[1], num) and \
            not self.validate.checkNumber(cell_B[0], cell_B[1], num) and \
            not self.validate.checkNumber(cell_C[0], cell_C[1], num)
